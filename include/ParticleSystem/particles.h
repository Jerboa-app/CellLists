...
bool handleCollision(float * X, uint64_t i, uint64_t j, float k = 300.0, float dt = 1.0/60.0);
...
uint64_t step(float * X, float * Xp, uint64_t * list, uint64_t * cells, float * offsets, float * thetas, std::vector<glm::vec2> attractors, std::vector<glm::vec2> repellors, std::default_random_engine generator, int N, int Nc, float delta, float L, float Dr, float r, float k, float v0, float dt);
